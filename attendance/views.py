    ...
    if exists('attendance.csv'):
        with open('attendance.csv', newline='') as file:
            ...
            if flag:
                ...
                print("First check in or You have already Check out")
                return False
    else:
...
def checkout(request):
    try:
        ...
        if(request.user.username == best_prediction):
            print("Valid")
            flag = markmyAttendanceOut(best_prediction)
            if flag == True:
                messages.add_message(request, 25, best_prediction + ', you have successfully checked-out.')
            else:
                messages.add_message(request, 25, best_prediction + ', Please check-in first.')
        else:
            print("Invalid")
            messages.add_message(
